...
public class MobileFrameworkDispatcher extends HttpServlet {
	...
	private static ActionConfigurations _actionConfig;
	...
	public void init(ServletConfig config) throws ServletException {    
		...
		if (listOfClasses != null && listOfClasses.length > 0) {
			...
			_actionConfig = MobileFrameworkHelper
		...
		try {
			if (_actionConfig != null) {
				...
				if (actionName.equals(this.lastModelDataAction)) {
					...
					// Invoke action
					Method method = _actionConfig.getMethod(actionName);
					Class<? extends WebActionHandler> actionClass = _actionConfig
